...
5. **Flash the Partitions**
   ...
   **Important:** Adjust the device names (/dev/sdb1, /dev/sdb2, etc.) according to your system.
   ...
   ```bash
   sudo dd if=part1.img of=/dev/mmcblk0p1 bs=4M status=progress
   sudo dd if=part2.img of=/dev/mmcblk0p2 bs=4M status=progress
   sudo dd if=part3.img of=/dev/mmcblk0p3 bs=4M status=progress
   ...
   sudo dd if=part22.img of=/dev/mmcblk0p22 bs=4M status=progress
   ```
